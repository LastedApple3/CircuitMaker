...
public partial class GUIForm : Form
{
    ...
    {
        ExtAppEditorForm dialog = new ExtAppEditorForm(new BoardContainerComponent(builder.GetBoard(), false), builder.colourScheme);
...
public struct ComponentOrWire
{
    ...
    public void CreateBoardComponent()
    {
        using (OpenFileDialog openFileDialog = new OpenFileDialog())
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                StartDraggingComponent(new BoardContainerComponent(intBoard, false), new Point(), true, true);
    ...
    {
        Dictionary<Pos, State> before = board.GetStateToCheckForChanges();

        board.Tick();

        Dictionary<Pos, State> after = board.GetStateToCheckForChanges();
